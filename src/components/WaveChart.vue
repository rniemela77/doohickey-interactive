<template>
    <div class="wave-chart">
        ...
        <svg width="100%" height="100%" viewBox="0 0 300 200" class="wave-svg">
            ...
            <!-- Wave 1 - Red -->
            <path class="wave wave1" :d="wave1Path" fill="none" stroke="red" stroke-width="3" :style="{
                opacity: 1 - (Math.abs(props.controlPosition.x - props.goalPosition.x) / 50),
            }" filter="drop-shadow(0 0 5px #ff0000)" />

            <!-- Wave 2 - Green -->
            <path class="wave wave2" :d="wave2Path" fill="none" stroke="green" stroke-width="3"
                :style="{ opacity: 1 - (Math.abs(props.controlPosition.y - props.goalPosition.y) / 50) }"
                filter="drop-shadow(0 0 5px #00ff00)" />

            <!-- Wave 3 - Blue -->
            <path class="wave wave3" :d="wave3Path" fill="none" stroke="white" stroke-width="3"
...
const wave1Speed = computed(() => goalSpeed + ((props.controlPosition.x - props.goalPosition.x) * 0.0005))
...
const wave2Speed = computed(() => goalSpeed + ((props.controlPosition.y - props.goalPosition.y) * 0.0005))

// Wave 3 (Blue) controls
const wave3Amplitude = goalAmplitude
const wave3Frequency = goalFrequency
const wave3Speed = goalSpeed
...
const wave3Path = computed(() => {
    const y0 = 100 + Math.sin(0 * wave3Frequency + time.value * wave3Speed) * wave3Amplitude
    let path = `M 0 ${y0}`
    for (let x = 2; x <= 300; x += 2) {
        const y = 100 + Math.sin(x * wave3Frequency + time.value * wave3Speed) * wave3Amplitude
        path += ` L ${x} ${y}`
    }
    return path
})
