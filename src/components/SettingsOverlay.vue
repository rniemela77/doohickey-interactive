<template>
    <div class="settings">
        <div class="settings-widget">
            <div class="settings-overlay column gap" v-if="isSettingsOpen">
                <button class="close-settings align-self-end" @click="isSettingsOpen = false">x</button>
                <h1>Settings</h1>
                <p>Adjusting these settings may improve performance:</p>
                <button class="settings-overlay-toggle" :class="{ 'enabled': showGlitchOverlay }" @click="showGlitchOverlay = !showGlitchOverlay">
                    Scanlines: {{ showGlitchOverlay ? 'On' : 'Off' }}
                </button>
                <button class="settings-overlay-toggle" :class="{ 'enabled': showNoiseOverlay }" @click="showNoiseOverlay = !showNoiseOverlay">
                    Noise: {{ showNoiseOverlay ? 'On' : 'Off' }}
                </button>
            </div>

            <!-- gear icon -->
            <button class="settings-toggle" @click="isSettingsOpen = !isSettingsOpen">
                <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
                        <path fill-rule="evenodd" clip-rule="evenodd"
                            d="M12.7848 0.449982C13.8239 0.449982 14.7167 1.16546 14.9122 2.15495L14.9991 2.59495C15.3408 4.32442 17.1859 5.35722 18.9016 4.7794L19.3383 4.63233C20.3199 4.30175 21.4054 4.69358 21.9249 5.56605L22.7097 6.88386C23.2293 7.75636 23.0365 8.86366 22.2504 9.52253L21.9008 9.81555C20.5267 10.9672 20.5267 13.0328 21.9008 14.1844L22.2504 14.4774C23.0365 15.1363 23.2293 16.2436 22.7097 17.1161L21.925 18.4339C21.4054 19.3064 20.3199 19.6982 19.3382 19.3676L18.9017 19.2205C17.1859 18.6426 15.3408 19.6754 14.9991 21.405L14.9122 21.845C14.7167 22.8345 13.8239 23.55 12.7848 23.55H11.2152C10.1761 23.55 9.28331 22.8345 9.08781 21.8451L9.00082 21.4048C8.65909 19.6754 6.81395 18.6426 5.09822 19.2205L4.66179 19.3675C3.68016 19.6982 2.59465 19.3063 2.07505 18.4338L1.2903 17.1161C0.770719 16.2436 0.963446 15.1363 1.74956 14.4774L2.09922 14.1844C3.47324 13.0327 3.47324 10.9672 2.09922 9.8156L1.74956 9.52254C0.963446 8.86366 0.77072 7.75638 1.2903 6.8839L2.07508 5.56608C2.59466 4.69359 3.68014 4.30176 4.66176 4.63236L5.09831 4.77939C6.81401 5.35722 8.65909 4.32449 9.00082 2.59506L9.0878 2.15487C9.28331 1.16542 10.176 0.449982 11.2152 0.449982H12.7848ZM12 15.3C13.8225 15.3 15.3 13.8225 15.3 12C15.3 10.1774 13.8225 8.69998 12 8.69998C10.1774 8.69998 8.69997 10.1774 8.69997 12C8.69997 13.8225 10.1774 15.3 12 15.3Z"
                            fill="#ffffff"></path>
                    </g>
                </svg>
            </button>
        </div>

        <GlitchyOverlay v-if="showGlitchOverlay" />
        <AppBg v-if="showNoiseOverlay" />
    </div>
</template>

<script setup>
import GlitchyOverlay from './GlitchyOverlay.vue';
import AppBg from './AppBg.vue';
import { ref } from 'vue';

const showGlitchOverlay = ref(true);
const showNoiseOverlay = ref(true);

const isSettingsOpen = ref(false);
</script>

<style scoped>
.settings-widget {
    position: fixed;
    bottom: 0;
    right: 0;
    z-index: 1000;
    display: flex;
    flex-direction: column;
    align-items: flex-end;
    gap: 1rem;

    .settings-toggle {
        width: 64px;
        height: 64px;
        cursor: pointer;
        border: none;
        background: none;
        border: 1px solid transparent;

        &:hover {
            border: 1px solid white;
            border-radius: 8px;
        }
    }

    svg {
        width: 50%;
        height: 50%;
    }
}

.settings-overlay {
    background: rgba(0, 0, 0, 1);
    padding: 2rem 1rem;
    border-radius: 8px;
    border-top: 10px solid white;

    .close-settings {
        position: absolute;
        top: 0;
        right: 0;
        font-size: 2rem;
        padding: 1rem;
        background: none;
        border: none;
        color: white;
        cursor: pointer;
        border: 1px solid transparent;
        border-radius: 8px;

        &:hover {
            border: 1px solid white;
        }
    }
}

.settings-overlay-toggle {
    background: none;
    border: 1px solid white;
    font-size: 1.2rem;
    padding: 1rem 2rem;
    border-radius: 8px;
    cursor: pointer;
    color: red;
    
    &:hover {
        background: white;
        color: black!important;
    }

    &.enabled {
        color: white;
    }
}
</style>